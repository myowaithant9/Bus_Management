...
$errorCount = 0;
	...
	if(isset($_POST['viewSubmit'])) 
		{			
			...
			if (!preg_match("^\w\/\d{1,5}\/\d{1,2}^", $driverId)) {
			...
			$phNumber=$_POST["phNumber"];
				if(!preg_match("/^(\+\d{1,3} ?)?(\(\d{1,5}\)|\d{1,5}) ?\d{3}?\d{0,7}( (x|xtn|ext|extn|pax|pbx|extension)?\.? ?\d{2-5})?$/i",$phNumber)) 
				{
			...
			if (empty($_POST["nrcNumber"])) {
				$nrcError = "NRC is required";
				$errorCount++;
			} else {
				$nrcNumber = $_POST["nrcNumber"];
				// check name only contains letters and whitespace
				if (!preg_match("^\d{1,2}\/\w\w\w\(\w\)\d{1,6}^", $nrcNumber)) {
...
$sql = "SELECT driverId,driverName,nrcNumber,phNumber,address from driver where driverId='$driverId'";
...
if($result)
{
	...
	$sql = "Update driver set  driverName='$driverName',nrcNumber='$nrcNumber',phNumber='$phNumber',address='$address' 
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <form role="form" action="driverUpdate.php" method="post">                        
                            <div class="form-group">
                               <label class="control-label" for="driverId">Driver ID</label>
                                <input type="text" class="form-control" name="driverId" id="driverId"   
								 value="<?php  if ( isset( $_POST["driverId"] ) || isset($_POST["updateSubmit"]) && $errorCount!=0) echo $_POST["driverId"]?>"/>
								 <span class='text-danger'> <?php echo $idError; ?> </span>	
                            </div>
							
							<div class="form-group">
                                <label class="control-label" for="driverName"> Driver Name</label>
                                <input type="text" class="form-control" name="driverName" id="driverName"  value="<?php  if ( isset( $_POST["viewSubmit"] )  || isset($_POST["updateSubmit"])) echo $driverName;?>">
                                <span class='text-danger'> <?php echo $nameError; ?></span>
                            </div>
                            
                            <div class="form-group">
                                <label class="control-label" for="nrcNumber">NRC Number</label>
                                <input type="text" class="form-control" name="nrcNumber" id="nrcNumber"  value="<?php  if ( isset( $_POST["viewSubmit"] ) || isset($_POST["updateSubmit"])) echo $nrcNumber;?>" >
                                <span class='text-danger'> <?php echo $nrcError; ?></span>
                            </div>
                            
                            <div class="form-group">
                                <label class="control-label" for="phNumber">Phone Number</label>
                                <input type="text" class="form-control" name="phNumber" id="phNumber"  value="<?php  if ( isset( $_POST["viewSubmit"] ) || isset($_POST["updateSubmit"])) echo $phNumber;?>">
